...
win = pygame.display.set_mode((550, 550)) # creez tabla

pygame.display.set_caption('Tic-Tac-Toe') # denumire titlu

first = pygame.draw.rect(win, (255,255,255), (25,25,150,150)) # deseneaza un dreptunghi, din cele 9
...
while run:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            run = False # cu acest if inchid fereastra

        if event.type == pygame.KEYDOWN: #resetare tabla la apasarea tastei space, resetare valori
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            pos = pygame.mouse.get_pos() #imi pastreaza pozitia atunci cand dau click

            if first.collidepoint(pos) and first_open: # daca dau click in prima cutie
                if draw_object == 'rect':
    ...
    pygame.display.update() # update imi aduce in fata ceea ce adaug( bring to front)
